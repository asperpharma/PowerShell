    ...
    End {
        ...
        if ($IncludeSymbols.IsPresent)
        {
            ...
            try
            {
                ...
                $dllAndExeFiles = $signingXml.SignConfigXML.job.file.src | Where-Object {  -not $_.endswith('pwsh.exe') -and ($_.endswith(".dll") -or $_.endswith(".exe")) } | ForEach-Object { ($_ -split '\\')[-1] }
                # Include both the original files and their corresponding .pdb files
...
function New-RpmSpec
{
    ...
    # Build spec content using array for better performance
    $specParts = @(
    ...
    if ($HostArchitecture -eq "x86_64" -or $HostArchitecture -eq "noarch") {
        $specParts += "BuildArch:      $HostArchitecture"
        $specParts += ""
    } else {
        ...
        $specParts += "%define __strip /bin/true"
        ...
        $specParts += "%global debug_package %{nil}"
        $specParts += ""
    }

    # Add dependencies
    foreach ($dep in $Dependencies) {
        $specParts += "Requires:       $dep"
    }
    ...
    foreach ($link in $LinkInfo) {
        ...
        $specParts += "mkdir -p `$RPM_BUILD_ROOT$linkDir"
        ...
        $specParts += "cp -P $($link.Source) `$RPM_BUILD_ROOT$($link.Destination)"
    }
    ...
    $specParts += @(
        ""
        "%changelog"
        "* $changelogDate PowerShell Team <PowerShellTeam@hotmail.com> - $rpmVersion-$rpmRelease"
        "- Automated build"
    )

    # Join all parts with newlines for final content
...
function New-NativeDeb
{
    ...
    try {
        ...
        # Calculate installed size (in KB) using Measure-Object for better performance
